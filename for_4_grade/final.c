...
void *process_manager(void *data)
{
    ...
    char *encrypted_text = (char *)calloc(BUFFER_SIZE * num_threads, sizeof(char));
    ...
    while (completed_threads < num_threads)
    {
        ...
        {
            int semaphore_value;
            sem_getvalue(&thread_data[i].semaphore, &semaphore_value);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_threads; i++)
    {
        ...
        sem_init(&thread_data[i].semaphore, 0, 0);
        pthread_create(&threads[i], NULL, encrypt_text, &thread_data[i]);
        thread_data[i].thread_id = threads[i];
    }

    sleep(1); // Allow the worker threads to start executing
    ...
    for (int i = 0; i < num_threads; i++)
    {
        sem_destroy(&thread_data[i].semaphore);
    }
